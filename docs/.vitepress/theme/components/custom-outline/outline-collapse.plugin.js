...
function setOutline() {
  ...
  if (aside) {
    ...
    for (let i = 0; i < outlineLinks.length; i++) {
      ...
      const outlineItems = outlineLinks[i].parentNode.querySelectorAll(
        ":scope > .VPDocOutlineItem"
      );

      if (outlineLinks[i].parentNode.querySelectorAll(".arrow").length)
        continue;
...
function setActive(active) {
  ...
  for (let i = 0; i < outlineLinks.length; i++) {
    ...
    const arrow =
      outlineLinks[i].parentNode.querySelectorAll(":scope > .arrow");
